...
{
    /// <summary>
    /// SolveFocalRatio is a class representing a page to calculate and display the focal ratio.
    /// </summary>
    public sealed partial class SolveFocalRatio : Page
    {
        /// <summary>
        /// Constructor for the SolveFocalRatio class.
        /// </summary>
        public SolveFocalRatio()
        {
        ...
        public IFormula CreateFormula(string formulaType, params double[] parameters)
        {
            ...
            {
                case "F-Number":
                    return new FocalRatio(parameters[0], parameters[1], parameters[2]);
        ...
        {
            // Get user input
            if (double.TryParse(FocalLengthTextBox.Text, out double f) && double.TryParse(DiameterTextBox.Text, out double D) && double.TryParse(DiameterTextBox.Text, out double N))
            {
                // Create the formula instance
                IFormula formula = CreateFormula("F-Number", f, D, N);

                // Perform the calculation
                double result = formula.Calculate();

                // Display the result
                ResultTextBlock.Text = $"Result: {result} F-Number";
            }
            else
            {
